...
public class TidyDifficulty3 : MonoBehaviour
{
    ...
    void Awake()
    {
        //temps d'attentre avant le d�rangement
        time = (int)Time.time;
        event1 = 300;
    }
    ...
    void Start()
    {
        //on recup�re le StreamWritter deja ouvert dans le script Oven
        sw = Oven.sw;
    ...
    {
        kids.Play();
        //D�sactiver tous les objets rang�s
    ...
    {
        // enregistrement du succes si la piece est rang� mais les oreillers pas dans le bon ordre
        if (!isHeaderWritten)
        {
            sw.Write("{0}\t{1}\t{2}\tPi�ce rang�e\tSucces\n", ID_partie, nom, prenom);
            sw.Write("{0}\t{1}\t{2}\tPlace oreillers\tEchec\n", ID_partie, nom, prenom);

            isHeaderWritten = true;
            sw.Flush();
            Debug.Log(string.Format("Pi�ce rang�e mais oreillers dans le mauvais ordre"));
        }
    }
    private void ExportFullSucces()
    {
        // enregistrement du succes si la piece est rang� avec les oreillers dans le bon ordre
        if (!isHeaderWritten)
        {
            sw.Write("{0}\t{1}\t{2}\tPi�ce rang�e\tSucces\n", ID_partie, nom, prenom);
            ...
            isHeaderWritten = true;
            sw.Flush();
            Debug.Log(string.Format("Piece rang�e et oreillers dans le bon ordre"));
        }
        sw.Close();
    }
}
